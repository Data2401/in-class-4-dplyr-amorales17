...
```{r}
starwars %>% group_by(eye_color = "brown") %>% summarise(mean(height))
```
...
```{r}
starwars %>% group_by(eye_color = "brown") %>% summarise(mean(height, na.rm = T))
...
```{r}
#tallest male
starwars %>% group_by(sex ="male") %>% summarize(max(height))
```
